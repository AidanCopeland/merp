# -*- coding: utf-8 -*-
"""
The Control Lycanthropy static maneuver table.

Classes:
    ControlLycanthropyManeuverTable

Functions:
    determine_moon_bonus(moon_phase)
    determine_day_night_bonus(time)
    determine_start_stop_bonus(start_stop)
    determine_taken_damage_bonus(taken_hits, taken_critical)
    determine_friend_damage_bonus(friend_damage)
"""
import sys
...
class ControlLycanthropyManeuverTable(SelfControlManeuverTable):
    """
    Control Lycanthropy static maneuver table.
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_taken_hits_frame():
            ...
            """
            trace.entry()
            frame_utils.setup_checkbox_frame(parent_frame, DAMAGE_TEXT, self.taken_damage)
            trace.exit()
        ...
        def setup_taken_critical_frame():
            ...
            """
            trace.entry()
            frame_utils.setup_checkbox_frame(parent_frame, CRITICAL_TEXT, self.taken_critical)
            trace.exit()
