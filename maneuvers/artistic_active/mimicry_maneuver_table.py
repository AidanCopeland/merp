# -*- coding: utf-8 -*-
"""
The Mimicry static maneuver table.

Classes:
    MimicryManeuverTable
"""
import sys
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_imitate_vocal_frame():
            ...
            frame_utils.setup_checkbox_frame(parent_frame,
                                             IMITATE_VOCAL_TEXT,
                                             self.imitate_vocal_patterns)
            trace.exit()

        def setup_multi_sided_frame():
            """
            Create a frame with a Checkbox indicating whether multi-sided conversations are
            imitated.
            """
            trace.entry()
            frame_utils.setup_checkbox_frame(parent_frame, MULTI_SIDED_TEXT, self.multi_sided)
            trace.exit()

        def setup_non_animate_frame():
            """
            Create a frame with an OptionMenu specifying the type of non-animate sounds being
            imitated
            """
            trace.entry()
            frame_utils.setup_optionmenu_frame(parent_frame,
                                               NON_ANIMATE_PROMPT,
                                               NON_ANIMATE_NONE_TEXT,
                                               self.non_animate,
                                               *NON_ANIMATE_OPTIONS)
            trace.exit()
