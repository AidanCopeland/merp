# -*- coding: utf-8 -*-
"""
The Tightrope Walking moving maneuver table.
...
class TightropeWalkingManeuverTable(MovingManeuverTable):
    """
    Tightrope Walking moving maneuver table.
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_wind_frame():
            ...
            """
            trace.entry()
            frame_utils.setup_entry_frame(parent_frame, WIND_TEXT, self.wind_bonus)
            trace.exit()
        ...
        def setup_surface_frame():
            ...
            """
            trace.entry()
            frame_utils.setup_entry_frame(parent_frame, SURFACE_TEXT, self.surface_bonus)
            trace.exit()
        ...
        def setup_balancing_pole_frame():
            ...
            """
            trace.entry()
            frame_utils.setup_checkbox_frame(parent_frame, BALANCING_POLE_TEXT, self.balancing_pole)
            trace.exit()

        def setup_tightrope_sole_shoes_frame():
            """
            Create a frame with a Checkbox indicating whether the character has tightrope sole
            shoes.
            """
            trace.entry()
            frame_utils.setup_checkbox_frame(parent_frame,
                                             TIGHTROPE_SOLE_SHOES_TEXT,
                                             self.tightrope_sole_shoes)
            trace.exit()
