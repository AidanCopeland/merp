...
class PaintingManeuverTable(ArtisticPassiveManeuverTable):
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_posed_frame():
            """
            Create a frame with a Checkbox indicating whether painting is from a posed image or person
            """
            trace.entry()
        ...
        def setup_ig_bonus_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, IG_BONUS_TEXT, self.ig_bonus)
            trace.exit()
