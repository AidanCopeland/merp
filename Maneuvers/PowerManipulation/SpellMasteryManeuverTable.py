...
class SpellMasteryManeuverTable(PowerManipulationManeuverTable):
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_caster_touch_frame():
            """
            Create a frame with a Checkbox indicating whether the range is increased
            from caster to touch.
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, CASTER_TO_TARGET_TEXT, self.caster_to_target)
            trace.exit()

        def setup_touch_10_foot_frame():
            """
            Create a frame with a Checkbox indicating whether the range is increased
            from touch to 10'.
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, TOUCH_TO_TEN_FT_TEXT, self.touch_to_10_foot)
            trace.exit()

        def setup_range_multiplier_frame():
            """
            Create a frame with an Entry indicating the range multiplier.
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, RANGE_MULTIPLIER_TEXT, self.range_multiplier)
            trace.exit()

        def setup_additional_target_frame():
            """
            Create a frame with an Entry indicating the number of additional targets.
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, ADDITIONAL_TARGET_TEXT, self.additional_targets)
            trace.exit()

        def setup_target_10_foot_frame():
            """
            Create a frame with a Checkbox indicating whether the area of effect is increased
            from a single target to 10'R.
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, TARGET_TO_TEN_FT_RADIUS_TEXT, self.target_to_10_foot)
            trace.exit()

        def setup_aoe_multiplier_frame():
            """
            Create a frame with an Entry indicating the area of effect multiplier.
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, AREA_OF_EFFECT_MULTIPLIER_TEXT, self.area_of_effect_multiplier)
            trace.exit()

        def setup_no_duration_to_1_rnd_frame():
            """
            Create a frame with a Checkbox indicating whether the duration is increased
            from no duration to 1 round.
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, NO_DURATION_TO_1_RND_TEXT, self.no_duration_to_1_rnd)
            trace.exit()

        def setup_concentration_to_rnd_per_level_frame():
            """
            Create a frame with a Checkbox indicating whether the duration is increased
            ...
            FrameUtils.setup_checkbox_frame(
                parent_frame, CONCENTRATION_TO_RND_PER_LVL_TEXT, self.concentration_to_rnd_per_lvl)
            trace.exit()

        def setup_concentration_to_min_per_level_frame():
            """
            Create a frame with a Checkbox indicating whether the duration is increased
            ...
            trace.entry()
            FrameUtils.setup_checkbox_frame(
                parent_frame, CONCENTRATION_TO_MIN_PER_LVL_TEXT, self.concentration_to_min_per_lvl)
            trace.exit()

        def setup_duration_multiplier_frame():
            """
            Create a frame with an Entry indicating the duration multiplier.
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, DURATION_MULTIPLIER_TEXT, self.duration_multiplier)
            trace.exit()

        def setup_increased_power_frame():
            """
        ...
        setup_additional_target_frame()
        setup_target_10_foot_frame()
        setup_aoe_multiplier_frame()
        self.additional_targets.set(0)
        self.area_of_effect_multiplier.set(1)

        setup_no_duration_to_1_rnd_frame()
        setup_concentration_to_rnd_per_level_frame()
        setup_concentration_to_min_per_level_frame()
        setup_duration_multiplier_frame()
        self.duration_multiplier.set(1)
        setup_increased_power_frame()
