...
from Maneuvers.StaticManeuverTable import \
    maneuver_difficulty_bonuses, ROUTINE, EASY, LIGHT, MEDIUM, HARD, VERY_HARD, EXTREMELY_HARD, SHEER_FOLLY, ABSURD
...
class JugglingManeuverTable(AthleticGymnasticsManeuverTable):
    ...
    def setup_maneuver_skill_frames(self, parent_frame):
        ...
        def setup_num_objects_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, NUM_OBJECTS_PROMPT, self.num_objects)
            trace.exit()
        ...
        def setup_irregular_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, IRREGULAR_PROMPT, self.is_irregular)
            trace.exit()
        ...
        def setup_sharp_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, SHARP_PROMPT, self.is_sharp)
            trace.exit()
        ...
        def setup_different_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_checkbox_frame(parent_frame, DIFFERENT_PROMPT, self.is_different)
            trace.exit()
        ...
        def setup_weight_frame():
            ...
            """
            trace.entry()
            FrameUtils.setup_entry_frame(parent_frame, WEIGHT_PROMPT, self.weight)
            trace.exit()
