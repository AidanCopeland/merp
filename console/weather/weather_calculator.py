"""
Creates the WeatherCalculator which calculates daily weather.
...
import sys
import os
from console.weather.weather_table import WeatherTable
import dice
...
class WeatherCalculator:
    """
    The Weather Calculator, which calculates daily weather based on the current weather table.

    Methods:
        __init__(self, standard_tables, weather_table)
        reset_variables(self)
        get_regions(self)
        set_region(self, region)
        read_modules(self)
        get_modules(self)
        calculate_weather(self)
        set_wind(self, wind_direction)
        get_wind_compass_direction(wind_direction)
        calculate_sunrise_sunset(self)
        calculate_sunrise()
        calculate_sunset()
    """
    ...
    def get_regions(self):
        """
        Returns the supported set of regions.
        """
        return sorted(self.regions.keys())

    def set_region(self, region):
        """
        Updates internal variables when the specified region changes.
        :param region: The region for which to calculate weather.
        """
    ...
    def read_modules(self):
        """
        Reads in the set of supported modules.
        """
    ...
    def get_modules(self):
        """
        Returns the supported set of modules.
        """
    ...
    def __calculate_min_temp(self):
        ...
        :return: Tuple of the daily minimum temperature in degrees Fahrenheit, and the modifier
        to apply to subsequent minimum temperature rolls.
        """
        temperature_roll = dice.d100high() + self.min_temp_roll_modifier
        trace.detail("Modified roll %d" % temperature_roll)

        temperature_mod_values = self.standard_tables.temperature_modification(False,
                                                                               temperature_roll)
        ...
        average_min_temp = \
            self.weather_table.get_average_min_temperature(self.current_area,
    ...
    def __calculate_max_temp(self):
        ...
        :return: Tuple of the daily maximum temperature in degrees Fahrenheit, and the modifier to
        apply to subsequent maximum temperature rolls.
        """
        temperature_roll = dice.d100high() + self.max_temp_roll_modifier
        trace.detail("Modified roll %d" % temperature_roll)

        temperature_mod_values = self.standard_tables.temperature_modification(True,
                                                                               temperature_roll)
        trace.detail("Ongoing modifier %d" % temperature_mod_values[1])
        ...
        average_max_temp = \
            self.weather_table.get_average_max_temperature(self.current_area,
                                                           self.standard_tables.month)
    ...
    def __check_for_ongoing_precipitation(self):
        """
        Checks whether there is any precipitation to carry over from the previous day, and updates
        current variables if so.
        :return: True if there is precipitation, False otherwise.
        """
        if self.ongoing_rainfall_duration_left > 0:
            trace.flow(
                "Prolonged rainfall event: duration %d" % self.ongoing_rainfall_duration_left)
            ...
            self.precipitation_duration = \
                self.precipitation_duration - (24 - self.precipitation_start)
    ...
    def __calculate_precipitation(self):
        ...
        :return: Tuple of the name of the precipitation and the index to the standard weather
        precipitation table for that precipitation type.
        """
        area_rainfall_chance = self.weather_table.get_rain_chance(self.current_area,
                                                                  self.standard_tables.month)
    ...
    def __prolonged_precipitation_next_day(self):
        """
        Pulls information about a prolonged precipitation event into the current variables after
    ...
    def __store_new_precipitation_info(self):
        ...
        self.precipitation_duration = \
            self.standard_tables.precipitation_duration(self.precipitation_type_index)
        self.precipitation_amount = \
            self.standard_tables.precipitation_amount(self.precipitation_type_index)
    ...
    def __calculate_wind(self):
        ...
        self.wind_speed = \
            self.standard_tables.wind_speed(self.precipitation_type_index) + \
            terrain_mod + altitude_mod

        self.wind_direction = self.__update_wind_direction(self.wind_direction)

    def set_wind(self, wind_direction):
        """
        Sets the wind direction to a specified value.
        Values accepted are N, NE, E, SE, S, SW, W, NW
    ...
    @staticmethod
    def __update_wind_direction(wind_direction):
        """
        ...
        # Determine the variation in the wind by using 4d90 to determine the alteration in degrees.
        # The midpoint, 182, is taken as meaning no change.
        dice_roll = dice.dcustom(90, 4)
        dice_roll = dice_roll - 182
    ...
    @staticmethod
    def get_wind_compass_direction(wind_direction):
        """
    ...
    def calculate_sunrise():
        """
        Get the current day of the year, as a number of days.
        Compare with spring equinox: 30 Gwaeron.  Result is x.
        Solar declination: delta = 23.5deg * sin [(x/365)*360def]
        Get latitude of observer.
        If abs(-tan (latitude)* tan (declination)) > 1, there is no sunrise or sunset.
        Calculate local hour angle of sunrise/sunset - degrees of earth's circumference between noon
        and sunrise/sunset.
