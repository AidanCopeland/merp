# coding=utf-8
"""
Creates the StandardTables which contains standard weather tables.
...
def celsius_from_fahrenheit(fahrenheit):
    ...
    """
    celsius = (old_div((fahrenheit - 32) * 5, 9))
    return celsius


def precipitation_start_time():
    """
...
class StandardTables:
    """
    Standard weather tables based on month.

    Methods:
        __init__(self)
        temperature_modification(self, high_temp, roll)
        set_terrain(self, terrain)
        terrain_min_temperature_modification(self)
        terrain_max_temperature_modification(self)
        set_altitude(self, altitude)
        mountain_temperature_modification(self)
        mountain_wind_modification(self)
        terrain_rain_chance_modification(self)
        terrain_wind_speed_modification(self)
        cloud_cover(self, is_raining)
        precipitation_type(self, min_temp, max_temp)
        precipitation_start_time()
        precipitation_duration(self, precipitation_index)
        is_precipitation_prolonged(self, precipitation_index)
        precipitation_amount(self, precipitation_index)
        wind_speed(self, precipitation_index),
        beaufort_info(self, wind_speed)
        get_month_index(self, month_name, date)
        get_days_past_spring_equinox(self)
        increment_date(self)
    """
    def __init__(self):
        ...
        self.temperature_modification_roll_chart = (
            # An array of information about modifiers to apply to the monthly average
            # minimum/maximum temperature based on the monthly variation class.

            # Each top-level array entry is a column in the Temperature Modification Chart for
            # variation classes 1 - 5 respectively (with an empty entry for class 0).

            # Each column entry is an array of entries in the column.  The array of entries
            # contains:
            # - Maximum resolved dice roll that matches this entry
            # - Modification (Fahrenheit) to apply to monthly average minimum/maximum
            # (reverse sign when applying to minimum)
        ...
        self.terrain_effects_chart = {
            # Dict containing modifiers to apply based on terrain.  Each entry contains an array,
            # where the entries specify:
            ...
            # Mountains have a temperature modification of -3F per 1000';
            # a wind modification of +5mph per 1000'
        ...
        self.cloud_cover_chart = (
            ...
            # Each entry contains the base % chance each month that the sky is clear,
            # or partly cloudy
            (0, 0),
        ...
        self.precipitation_inches_chart = (
            # Array containing rainfall amount for a rainfall event.
        ...
        self.beaufort_scale_chart = (
            ...
            (7, 38, "High wind, moderate gale, near gale",
    ...
    def temperature_modification(self, high_temp, roll):
        ...
        :param high_temp: Whether to look for the high temperature modification (or the low
        temperature modification)
        :param roll: The modified temperature roll.
        :return: Tuple of the modifier to apply to temperature, and a modifier to ongoing
        temperature rolls.
        """

        if high_temp:
            modification_class = self.__high_temperature_modification_class()
        else:
            modification_class = self.__low_temperature_modification_class()
    ...
    def __high_temperature_modification_class(self):
        """
    ...
    def __low_temperature_modification_class(self):
        """
    ...
    def mountain_temperature_modification(self):
        ...
        """
        return round(old_div((3 * self.altitude), 1000))
    ...
    def mountain_wind_modification(self):
        ...
        """
        return round(old_div((5 * self.altitude), 1000))
    ...
    def cloud_cover(self, is_raining):
        """
    ...
    def precipitation_duration(self, precipitation_index):
        ...
        :param precipitation_index: The index to self.precipitation_type_chart for the precipitation
        event.
        ...
        # This relies on duration_class taking values 1 - 3 and the 0th index being taken by the
        # dice roll.
    ...
    def precipitation_amount(self, precipitation_index):
        ...
        trace.detail("Amount %r inches" % amount)
        return amount
    ...
    def increment_date(self):
        ...
        # This dict is keyed by month name.  The tuple contains the month index (used for weather
        # calculation); the number of days in the month, and name of the next month.
        self.month_dict = {
